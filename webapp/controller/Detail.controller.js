sap.ui.define([
	"com/test/northwind/util/Controller",
	...
	return Controller.extend("com.test.northwind.controller.Detail", {
		/**
		...
		onInit: function() {
			...
			if (!this._detailReadMode) {
				...
				this._detailReadMode = sap.ui.xmlfragment(this._detailReadModeFragmentName, "com.test.northwind.view.DetailReadMode", this);
				this.switchMode("read");
			}

			if (!this._detailEditMode) {
				this._detailEditModeFragmentName = this.getView().getId() + "detailEditMode";
				this._detailEditMode = sap.ui.xmlfragment(this._detailEditModeFragmentName, "com.test.northwind.view.DetailEditMode", this);
			}

			if (!this._detailCreateMode) {
				this._detailCreateModeFragmentName = this.getView().getId() + "detailCreateMode";
				this._detailCreateMode = sap.ui.xmlfragment(this._detailCreateModeFragmentName, "com.test.northwind.view.DetailCreateMode", this);
			}
		...
		openDialog: function(sI18nKeyword) {
			if (!this._dialog) {
				...
				this._dialog = sap.ui.xmlfragment(frgId, "com.test.northwind.view.MsgDialog", this);
		...
		openCancelConfirmDialog: function() {
			if (!this._cancelConfirmDialog) {
				...
				this._cancelConfirmDialog = sap.ui.xmlfragment(frgId, "com.test.northwind.view.CancelConfirmDialog", this);
		...
		openDeleteConfirmDialog: function() {
			if (!this._deleteConfirmDialog) {
				...
				this._deleteConfirmDialog = sap.ui.xmlfragment(frgId, "com.test.northwind.view.DeleteConfirmDialog", this);
